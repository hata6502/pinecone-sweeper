...
export const Minesweeper: FunctionComponent = () => {
  ...
  const placeMines = useCallback((firstRow: number, firstCol: number) => {
    ...
    const safeZone = [
      [firstRow - 1, firstCol - 1], [firstRow - 1, firstCol], [firstRow - 1, firstCol + 1],
      [firstRow, firstCol - 1], [firstRow, firstCol], [firstRow, firstCol + 1],
      [firstRow + 1, firstCol - 1], [firstRow + 1, firstCol], [firstRow + 1, firstCol + 1]
  ...
  // タッチ／マウス離す
  const handleCellRelease = useCallback((row: number, col: number) => {
    if (longPressTimer) {
  ...
  const DifficultyButton = ({ level, label }: { level: 'easy' | 'medium' | 'hard', label: string }) => (
    <button
      type="button"
      className={clsx(
        // 基本スタイル
    ...
    <div className="flex flex-col items-center mx-auto">
      ...
      <div className={clsx(
        ...
        <div className="px-4 py-4 sm:px-6">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <div className="font-mono font-medium text-neutral-900 bg-neutral-100 px-2.5 py-1.5 rounded-md">
                {minesLeft.toString().padStart(3, '0')}
              </div>
            </div>
            <button 
              className="bg-neutral-50 w-10 h-10 flex items-center justify-center rounded-full text-2xl shadow-sm hover:bg-neutral-100 border border-neutral-200"
        ...
        <div className="px-4 py-5 sm:p-6">
              ...
              <div key={rowIdx} className="flex gap-1">
                {row.map((cell, colIdx) => (
                  <div
                    key={`${rowIdx}-${colIdx}`}
                    className={clsx(
                      "w-8 h-8 flex items-center justify-center font-bold rounded-md transition-all",
                      getCellColor(cell),
                      cell.isRevealed 
                        ? "border border-zinc-200" 
                        : "border border-zinc-300 cursor-pointer shadow-sm hover:border-zinc-400"
                    )}
                    onMouseDown={() => handleCellPress(rowIdx, colIdx)}
                    onMouseUp={() => handleCellRelease(rowIdx, colIdx)}
                    onMouseLeave={() => {
                      if (pressedCell && pressedCell.row === rowIdx && pressedCell.col === colIdx) {
                        if (longPressTimer) {
                          clearTimeout(longPressTimer);
                          setLongPressTimer(null);
                        }
          ...
          {gameState !== 'playing' && (
            <div className={clsx(
              "mt-6 p-3 rounded-lg text-center font-medium border",
              gameState === 'won' 
                ? "bg-green-50 text-green-700 border-green-200" 
                : "bg-red-50 text-red-700 border-red-200"
            )}>
              <span className="font-semibold">{gameState === 'won' ? 'ゲームクリア！おめでとう！' : 'ゲームオーバー！再挑戦しよう'}</span>
            </div>
          )}
        </div>
      </div>
